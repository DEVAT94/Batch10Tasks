...
void main() {
  ...
  print(
    'Wilkommen $blueText${names[1]}$whiteText, bist du bereit dich ins Abenteuer zu stützen?',
  );
  print(lineBreak);
  print(
    'Zu aller erst gib bitte deinen Namen ein. Ich möchte dich ungern den Rest des Abenteuers $blueText${names[1]}$whiteText nennen müssen.\n',
  );
  ...
  if (names[1] == null || names[1] == '') {
    ...
    print(lineBreak);
    print(
      'Nun, wenn du mir deinen Namen nicht veraten möchtest, nenne ich dich eben weiter $blueText PlAyEr $whiteText ((>*_*)> lame <(*_*<))',
    );
  } else {
    print(lineBreak);
  ...
  print(lineBreak);
  print(
    '''Nun $blueText${names[1]}$whiteText, deine Reise beginnt hier in den ${yellowText}Ruinen von Darnok${whiteText}. Genauergesagt in einer alten 
...
Dein Ziel ist klar: Finde $blueText${names[0]}$whiteText, stelle dich ihm im Kampf und befreie die Festung von seinem Fluch.''',
  ...
  print(lineBreak);
  print(
    'Du betrittst die Ruinen. $blueText${names[0]}$whiteText steht voller Hass vor dir und greift dich an.',
  );
  print(lineBreak);
  print('Was tust du?');
  print(actionOptions);
  ...
  do {
    switch (stdin.readLineSync()) {
      case '1':
        ...
        print(
          '$blueText${names[0]}$whiteText erleidet $redText$dmg$whiteText Schaden.\n',
        );
        ...
        if (bossHealth <= 0) {
          ...
          print(lineBreak);
          print(
            'Herzlichen Glückwunsch Kleiner, du hast die Welt gerettet. Der Name $blueText${names[1]}$whiteText soll von nun an, jedem auf der Welt bekannt sein!',
          );
          defaultSwitch = true;
        } else {
          dmg = rnd.nextInt(10);
          ...
          print(
            '$blueText${names[0]}$whiteText setzt zum Gegenschlag an. Du erleidest $redText$dmg$whiteText Schaden!',
          );
          print(lineBreak);

          if (playerHealth <= 0) {
            print(
              '${redText}Game over$whiteText Sportsfreund. Die Welt ist verdammt! $blueText${names[0]}$whiteText wird alles und jeden vernichten! Muhaha',
            );
          } else {
            print('Was tust du?');
            print(actionOptions);
      ...
      case '2':
        ...
        print(lineBreak);
        print(
          'Du trinkst einen Heiltrank! Du hast $greenText$healthRestored$whiteText Lebenspunkte wiederhergestellt. Du hast $greenText$playerHealth$whiteText Leben! Du hast noch $greenText${--potion}$whiteText verfügbar\n',
        );
  ...
  } while (defaultSwitch == false && bossHealth >= 0 ||
      defaultSwitch == false && playerHealth >= 0);
